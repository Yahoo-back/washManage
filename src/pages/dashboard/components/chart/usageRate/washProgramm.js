...
class WashProgramm extends React.Component {
  query = async (store, sTime, eTime) => {
    this.props.dispatch({
      type: 'dashboard/getWashRate',
      payload: {
        endTime: eTime,
        startTime: sTime,
        storeName: store,
  ...
  render() {
    const { dashboard: { washRateData } } = this.props
    let data = washRateData && washRateData.dataMap
    let i = 0
    let chartData = []
    for (let p in data) {
      chartData[i] = {
    ...
    const option = {
      color: ['#55cd64', '#febc34', '#11a0f7', '#745af0', '#f52d52'],
      tooltip: {
        trigger: 'axis',
        ...
        ${params[4] ? (params[4].seriesName + ':' + params[4].value) : ''}<br/>
      `)
      },
      grid: {
        left: '0',
      ...
      series: [
        ...
        {
          name: chartData[3] ? chartData[3].kind : '',
          type: 'line',
          smooth: true,
          symbol: 'circle',
          symbolSize: 7,
          ...
          data: chartData[3] ? chartData[3].num : ''
        },
        {
          name: chartData[4] ? chartData[4].kind : '',
          type: 'line',
          smooth: true,
          symbol: 'circle',
          symbolSize: 7,
          ...
          data: chartData[4] ? chartData[4].num : ''
        }
      ]
    }
...
WashProgramm.propTypes = {
  dashboard: PropTypes.object,
  dispatch: PropTypes.func,
}

export default connect(({ dashboard }) => ({ dashboard }))(WashProgramm)